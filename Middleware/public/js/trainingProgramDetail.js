import { getTrainingProgramById } from "./Services/trainingprogram.service.js";
...
$(document).ready(function () {
  var checkId = sessionStorage.getItem("programId");
  getTrainingProgramById(checkId).then(function (trainingProgram) {
    var nameProgram = trainingProgram.name;
    var statusProgram = trainingProgram.status;
    var durationProgram = trainingProgram.duration;
    var infoProgram = trainingProgram.information;
    var createdProgram = trainingProgram.createOn;
    var createBy = trainingProgram.createBy;
    var syllabusProgram = trainingProgram.syllabuses;
    console.log(statusProgram);
    $("#nameProgram").text(nameProgram);
    if (statusProgram) {
      $("#statusProgram").text("Active");
    } else {
      $("#statusProgram").text("Inactive");
    }
    $("#durationTime").text(durationProgram + " days");

    $("#infoProgram").html(infoProgram);
    $("#createTime").text(
      "Modified on " + createdProgram + " create by " + createBy.name
    );

    syllabusProgram.forEach(function (checkSyllabusId) {
      getSyllabus(checkSyllabusId).then(function (syllabus) {
        getUser(syllabus.modifiedBy).then(function (user) {
          var syllabusName = syllabus.name;
          var syllabusVersion = syllabus.version;
          var syllabusModifiedBy = user.name;
          var syllabusModifiedOn = syllabus.modifiedOn;
          var syllabusStatus = syllabus.status;
          var syllabusDuration = syllabus.trainingTime;
          var syllabusDays = Math.ceil(syllabusDuration / (24 * 60));
          var syllabusHours = syllabusDuration / 60;

          var tableHtml =
            '<div style="border-style: solid; border-radius: 25px; padding: 10px; margin-left: 20px;">';
          tableHtml +=
            '<div style= "display: flex; align-items: center; justify-content: flex-start; margin-left: 20px;">';
          tableHtml +=
            `<h4> ${syllabusName} </h4>` +
            `<div style="border-style: solid; border-radius: 25px; padding: 5px 10px; margin-left: 10px;"> ${statusEnum[syllabusStatus]} </div>`;
          tableHtml += "</div>";
          tableHtml +=
            '<div style="border-bottom-style: solid; padding: 10px; margin-left: 20px;">';
          tableHtml += `<div> v${syllabusVersion} | ${syllabusDays} days (${syllabusHours} hours) | Modified on ${syllabusModifiedOn} by ${syllabusModifiedBy}</div>`;
          tableHtml += "</div>";
          tableHtml += generateSyllabusDays(syllabus);
  ...
  materials.forEach((material) => {
    contentMaterial += `<div class="material-list">
                      <div class="material-link m-3 row">
                          ...
                          <span class="col text-right">by ${
                            material.owner
                          } on ${material.createdDate}</span>
                      </div>
                      </div>`;
  });
  contentMaterial += `</div>
...
function generateUnitContent(day, unit, content) {
  let unitContent = `<li class="d-flex flex-row my-1 p-1 overflow-x-scroll" style="border-radius: 10px; background-color: lightgray;">
    <span class="flex-grow-1 my-auto ms-2 fw-medium text-nowrap">${
      content.name
    }</span>
    <span class="flex-shrink-1 mx-1 pill-button fw-medium text-nowrap overflow-x-hidden">${
      content.outputStandard
    }</span>
    <span class="flex-grow-1 mx-1 pill-button fw-medium text-nowrap">${
      content.trainingTime + " minute(s)"
    }</span>
    <span class="flex-shrink-1 mx-1 pill-button fw-medium">${
      deliveryMethodEnum[content.deliveryMethod]
    }</span>
...
function generateDayUnit(day, unit) {
  let dayUnit = `<li class="row mt-0 m-3" style="border-bottom:1px solid;">
    <div class="col-md-1 col-2 fw-bold fs-5 text-nowrap">${
    ...
    <div class="col-md-11 col-10 row me-0 pe-0">
        ...
        <div class="col-1 me-0 pe-0 text-end">
            <button class="btn" data-bs-toggle="collapse" data-bs-target="#collapse-unit-${
              unit.unitNumber
            }" aria-expanded="false" aria-controls="collapse-unit-${
    unit.unitNumber
  }">
        ...
        <ul class="col-12 collapse show" id="collapse-unit-${
          unit.unitNumber
        }">`;
  ...
  let syllabusDay = `<li class="mb-3 mx-2">
    <div class="input-group input-group-lg">
        <button class="btn btn-dark flex-grow-1 text-start" type="button"
            ...
            aria-expanded="false" aria-controls="day-${
              day.dayNumber
            }-content" id="headingOne">
            ${"Day " + day.dayNumber}
        </button>
    </div>
    <div class="collapse show" id="day-${
      day.dayNumber
    }-content" style="border:1px solid gray;border-radius:5px; box-shadow: 5px 5px 5px;">
...
function generateSyllabusDays(syllabus) {
  var syl = `<ul id="syllabus-days-box" class="p-0 m-2" style="max-height:30vh; overflow-y:scroll; list-style-type: none; border: 1px solid;border-radius:10px ">
